...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        self.isNeedVoiceButton = YES;
        self.isNeedAddButton = YES;
    }
    return self;
}
#pragma mark - 图片按钮点击
- (void)plusBtnClick {
    ...
    if (!self.isEditing) { //没有弹出键盘,点击加号弹出键盘
        ...
        //NSString *offset = [NSString stringWithFormat:@"%f",258.0+49.0];
...
//        self.isTextMaxH = NO;
//        self.backGroundView.frame = CGRectMake(0, 0, App_Width, self.inputH);
//        NSLog(@"backGroundView  %@",NSStringFromCGRect(self.backGroundView.frame));
//        NSLog(@"textView  %@",NSStringFromCGRect(textView.frame));
//        UIView *contentView = textView.subviews.firstObject;
//        NSLog(@"contentView  %@",NSStringFromCGRect(contentView.frame));
//    }else{
//        self.isTextMaxH = YES;
//        return;
//    }
////    NSLog(@"backGroundView  %@",textView.subviews);
//    
//    //[textView.inputView sizeToFit];
//    self.frame = CGRectMake(0, App_Height - self.keyboardHeight - self.inputH, App_Width,self.inputH + self.keyboardHeight );
//    
//}
-(void)TextViewDidChange:(CGSize)size{
    ...
    self.placeholderLabel.text =  size.width == 0 ? @"说点什么吧...":@"";
    CGFloat curheight = size.height;
    self.inputH = curheight + 12;
    if (self.currentTextViewH != curheight && curheight < 80) {
        self.currentTextViewH = curheight;
        NSString *offset = [NSString stringWithFormat:@"%f",self.keyboardHeight + self.inputH];
        [[NSNotificationCenter defaultCenter]postNotificationName:@"textInputFrameChange" object:nil userInfo:@{@"offset":offset}];
...
- (void)setUI:(UIViewController *)parentVC{
    ...
    [self addSubview:self.backGroundView];//添加 输入框背景
    [self.backGroundView addSubview:self.textView];//添加 输入框
    [self.backGroundView addSubview:self.placeholderLabel];//添加 输入框默认字段
    if (self.isNeedAddButton) {
        [self.backGroundView addSubview:self.plusBtn];//添加 加号按钮
        self.picBtn = [[UIButton alloc]init];
        [self.picBtn setImage:[UIImage imageNamed:@"keyboard_pic"] forState:UIControlStateNormal];
        [self addSubview:self.picBtn];//添加 照片按钮
        ...
        [self addSubview:self.photoBtn];//添加 相机按钮
        self.photoBtn.hidden = YES;
    }
    if (self.isNeedVoiceButton) {
        [self.backGroundView addSubview:self.voiceBtn];//添加 录音按钮
    }
...
- (UITextView *)textView{
    if (!_textView) {
        _textView = [[KMPlaceholderTextView alloc] initWithFrame:CGRectMake(0, 0, 0, 0) PlaceText:@"" PlaceColor:[UIColor blackColor]];
        _textView.font = [UIFont systemFontOfSize:16];
        _textView.delegate = self;
        _textView.layer.cornerRadius = 5;
        _textView.returnKeyType =  UIReturnKeySend;
        [_textView TextViewSizeChange:^(CGSize size) {//拿到内容的改变值
            if (!self.textView.isFrist) {
                self.textView.isFrist = YES;
            }else{
