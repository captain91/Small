...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(20.0f, NAV_TAB_BAR_HEIGHT - 2.0f, width - 44.0f, 2.0f)];
    /**
     *  设置下划线颜色
     */
...
//    [_navgationTabBar setBackgroundColor:[UIColor redColor]];
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        button.frame = CGRectMake(buttonX, DOT_COORDINATE, [widths[index] floatValue]-1, NAV_TAB_BAR_HEIGHT);
        [button setTitle:_itemTitles[index] forState:UIControlStateNormal];
        [button setTitleColor:kUIColorFromRGB(0x323232) forState:UIControlStateNormal]; // 标题字体颜色
//        [button setBackgroundColor:[UIColor redColor]];
        
        button.titleLabel.font=[UIFont systemFontOfSize:16];
        button.tag=140+index;
        [button addTarget:self action:@selector(itemPressed:) forControlEvents:UIControlEventTouchUpInside];
//        button.titleLabel.font = [UIFont systemFontOfSize:14]; // 标题字体大小
        [_navgationTabBar addSubview:button];
        
        if (index==0) {
            [button setTitleColor:kUIColorFromRGB(0xe93030) forState:UIControlStateNormal];
        }
...
- (void)itemPressed:(UIButton *)button
{
    ...
    for (NSInteger i=0; i<_items.count; i++) {
        if (i==index) {
            ...
            [_items[i] setTitleColor:kUIColorFromRGB(0x323232) forState:UIControlStateNormal];
        }
    }
...
//        CGSize size = [title sizeWithFont:[UIFont systemFontOfSize:[UIFont systemFontSize]]];
        ...
        NSNumber *width;
        if (SCREEN_WIDTH>320) {
            width = [NSNumber numberWithFloat:size.width +SCREEN_WIDTH/6];
        }else
            width = [NSNumber numberWithFloat:size.width +SCREEN_WIDTH/7];
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        _line.frame = CGRectMake(button.frame.origin.x + 20.0f, _line.frame.origin.y, [_itemsWidth[currentItemIndex] floatValue] - 44.0f, _line.frame.size.height);
    }];
}

- (void)updateData
{
    _arrowButton.backgroundColor = [UIColor redColor];
